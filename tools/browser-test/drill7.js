...
(async () => {
  ...
  // capture all layout API requests
  let lastLayoutReq = null;
  page.on('request', req => {
    if (req.url().includes('/api/graph/layout')) {
      try { lastLayoutReq = JSON.parse(req.postData() || '{}'); } catch(_){}
    }
  });
  ...
  await page.waitForTimeout(500);
  console.log('last layout request:', JSON.stringify(lastLayoutReq, null, 2));
  await browser.close();
})();
