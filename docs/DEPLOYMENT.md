...
| `GRAPHDEN_REGISTRY_URL` | *(empty)* | The remote registry `GRAPHDEN_REGISTRY_TOKEN` belongs to (e.g. `https://graphden.dev`). The bearer is sent ONLY on dials whose origin (scheme + host + port) is this one — a remote install's `source` is caller-chosen, so any other host is dialed without it. Unset ⇒ the token is never sent. A public `:deploy-config` setting |
| `GRAPHDEN_REGISTRY_TOKEN` | *(empty)* | Bearer used when pulling a package from the remote registry named by `GRAPHDEN_REGISTRY_URL` (`POST /api/packages/install` with `source` — [PACKAGE_DISTRIBUTION § 13](PACKAGE_DISTRIBUTION.md#13-self-hosted-install-by-package-type)). Empty ⇒ remote fetches go unauthenticated |
...
| `GRAPHDEN_HUB_TOKEN` | *(empty)* | Bearer this instance presents to the hub (only on dials to `GRAPHDEN_HUB_URL`'s origin) for `/api/sync/*` push/pull (mint it on the hub: `/api/my-tokens` on a cloud org, or its `AUTH_TOKEN` on a bare self-hosted hub). Never surfaced to browsers |
| `GRAPHDEN_FEEDBACK_URL` | *(unset — official intake)* | Intake URL the editor's **Report a problem** form POSTs to, announced via `GET /api/feedback/config`. Blank (unset or empty) ⇒ the official intake (`https://app.graphden.dev/api/feedback`); set to your own intake to redirect; set to the literal `off` to hide the feedback affordance entirely (air-gapped installs). Server config only — see [MONITORING § 4](MONITORING.md#4-user-feedback-intake) |
| `GRAPHDEN_FEEDBACK_INTAKE` | *(empty)* | Non-blank ⇒ THIS instance accepts feedback reports on open `POST /api/feedback` (stores into the `feedback_reports` SQL table; the built-in alerter pings the operator on its next tick — [MONITORING § 3b](MONITORING.md#3b-built-in-domain-alerter-opt-in--telegram-or-webhook)). Leave unset everywhere except the one instance that is the intake |
