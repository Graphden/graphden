...
(deftest test-rename-arg-back-ref-node
  (let [r (iface/add-args-back-ref (impl/init) :old-node :arg-a)
...
(deftest test-delete-arg-back-ref-node-removes-entry
  (let [r (impl/init {:args-back-refs {:n1 #{:a :b}}})
        r2 (impl/delete-arg-back-ref-node r :n1 :a)]
    (is (= #{:b} (get-in r2 [:args-back-refs :n1])))))

(deftest test-delete-arg-back-ref-node-removes-key-if-empty
  (let [r (impl/init {:args-back-refs {:n1 #{:only}}})
        r2 (impl/delete-arg-back-ref-node r :n1 :only)]
    (is (nil? (get-in r2 [:args-back-refs :n1])))
    (is (= {} (:args-back-refs r2)))))

(deftest test-delete-arg-back-ref-node-does-nothing-if-arg-missing
  (let [r (impl/init {:args-back-refs {:n1 #{:a}}})
        r2 (impl/delete-arg-back-ref-node r :n1 :z)] ; :z не существует
    (is (= #{:a} (get-in r2 [:args-back-refs :n1])))))

(deftest test-delete-arg-back-ref-node-does-nothing-if-node-missing
  (let [r (impl/init {:args-back-refs {:n1 #{:a}}})
        r2 (impl/delete-arg-back-ref-node r :nonexistent :a)] ; :nonexistent нет
    (is (= {:n1 #{:a}} (:args-back-refs r2)))))
