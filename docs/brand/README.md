...
The three text-bearing images (`boosty`, `youtube`, `social-preview`) are
generated by `gen_banners.py` (PIL; stock DejaVu fonts). The tagline they
carry is the canonical one-liner from
[PHILOSOPHY § Positioning](../PHILOSOPHY.md#positioning) — when it changes,
edit `TAGLINE` in the script, re-run
`python3 docs/brand/gen_banners.py docs/brand`, and re-upload to each
platform. The text-free backdrops (`x`, `linkedin`) never need regeneration.
