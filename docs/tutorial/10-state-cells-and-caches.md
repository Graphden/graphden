# Lesson 10 — State: cells, swap, and a graph-native cache
...
constant (lesson 04's `:value`), so the *same* box is handed back on
...
of the same type. Now `▶ Run` (via `⋯`) on `:count-a-hit` (lesson 13) returns `1`,
...
`:swap` and `:reset` carry the `:state` effect (lesson 14): they
...
a load balancer (lesson 33's cardinality, [SCALING.md](../SCALING.md))
...
[lesson 11 — Recursion: loops without cycles](11-recursion.md)
