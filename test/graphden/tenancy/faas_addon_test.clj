...
(deftest faas-fragment-merges-over-addon
  (let [cfg (config/read-config :test ["graphden/tenancy/addon.edn"
    ...
    (testing "app-router key present and wired to the org-resolver"
      (is (contains? cfg :tenancy/app-router))
      (is (= (ig/ref :tenancy/org-resolver)
             (:org-resolver (:tenancy/app-router cfg)))))
    (testing "base-domain resolves (env default) and timeout carries"
      (is (string? (:base-domain (:tenancy/app-router cfg))))
    ...
    (testing "org schema splices into :db/schema extensions"
