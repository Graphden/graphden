...
| `GRAPHDEN_FEEDBACK_INTAKE` | *(empty)* | Non-blank ⇒ THIS instance accepts feedback reports on open `POST /api/feedback` (stores into the `feedback_reports` SQL table, pings the alerter's Telegram pair). Leave unset everywhere except the one instance that is the intake |
