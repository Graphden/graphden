(ns graphden.executor.registry.macros-test
...
(deftest defbase-expansion-test
  (testing "expands simple function without docstring"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase test-add
                                                                 {:args {:a :int, :b :int}
                                                                  :return-type :int}
                                                                 (+ a b)))]
  ...
  (testing "expands function with docstring"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase test-fn
                                                                 "This is a docstring."
                                                                 {:args {:x :text}
                                                                  :return-type :text}
                                                                 x))]
  ...
  (testing "expands HOF with :fn type arg"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase test-map
  ...
  (testing "handles optional args with map spec"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase test-opt
                                                                 {:args {:required :int
...
(deftest defbase-function-execution-test
  (testing "defined function works with delay args"
    ...
    (eval '(do
             (require '[graphden.executor.registry.macros :refer [defbase]])
...
(deftest edge-cases-test
  ...
  (testing "fn args with optional map spec are handled"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase test-fn-opt
  ...
  (testing "empty args map"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase const-fn
                                                                 {:args {}
                                                                  :return-type :int}
                                                                 42))]
      (is (= 'def (first expanded)))
  ...
  (testing "symbol arg names (not keywords)"
    ...
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase sym-arg-fn
                                                                 {:args {x :int, y :int}
                                                                  :return-type :int}
                                                                 (+ x y)))]
      (is (= 'def (first expanded)))
...
(deftest defbase-impl-source-test
  (testing "defbase includes :impl-source with original body"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase impl-source-fn
                                                                 {:args {:a :int, :b :int}
                                                                  :return-type :int}
                                                                 (+ a b)))
  ...
  (testing "defbase preserves multiple body forms in :impl-source"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase multi-body-fn
                                                                 {:args {:x :int}
                                                                  :return-type :int}
                                                                 (println "debug")
                                                                 (inc x)))
  ...
  (testing "defbase :impl-source is quoted (not evaluated)"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase quoted-fn
                                                                 {:args {:coll :jsonb}
                                                                  :return-type :int}
                                                                 (count coll)))
          fn-map (nth expanded 2)
  ...
  (testing "defbase with docstring includes :impl-source"
    (let [expanded (macroexpand-1
                     '(graphden.executor.registry.macros/defbase doc-impl-fn
