...
function nextPath(){const n=new URLSearchParams(location.search).get('next');if(!n||!/^\/(?!\/)/.test(n)||/[\\]/.test(n))return'/';try{const u=new URL(n,location.origin);if(u.origin!==location.origin||!/^https?:$/.test(u.protocol))return'/';return u.pathname+u.search+u.hash;}catch(_){return'/';}}
function goNext(){location.assign(new URL(nextPath(),location.origin).href);}
