# Lesson 31 — Packages: namespaces, fns.edn, impls.clj, deps
...
| `:services` | Optional — a package can declare a fn to keep running (the `app` package declares `:web-server` this way). See [Lesson 35](35-services.md) |
...
strips show the full set. See [Lesson 16](16-effects-and-secrets.md).
...
publish / install / update / fork — is [Lesson 32](32-distributing-packages.md).)
...
Unlike a registry install ([Lesson 32](32-distributing-packages.md)),
...
- **Distributing a package as data** (not on disk) — publish it
  into the graph and install it on a branch: [Lesson 32](32-distributing-packages.md).

## Next

[Lesson 32 — Distributing packages](32-distributing-packages.md):
