# Lesson 21 — Branches: fork, edit, diff, merge
...
[lesson 22](22-review.md)'s subject; its knobs live in the same
**⚙ Protection…** dialog.
...
10. From `main`, click `⇢` next to `tutorial-branch`. Confirm. The
    ...
    history and can no longer be deleted — [lesson 22](22-review.md)
    explains why.)
...
type diagnostics (what the Explorer's ⚠ type errors filter marks — Lesson 17),
...
- Per-branch services (running `:my-web-server` as a managed
  service on dev while `:web-server` runs on main) — Lesson 33.
...
[Lesson 22 — Review: propose, approve, and the protected merge](22-review.md):
