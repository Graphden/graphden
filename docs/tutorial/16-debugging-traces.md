# Lesson 16 — Debugging: traces, the call tree, and catching a request
...
`slugify` test subject from lesson 15, or any page fn):
...
Traces obey the same `:secret` rules as results (lesson 14):
