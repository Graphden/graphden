# Lesson 07 — Higher-order functions and `:fn`-typed slots
...
3. **Bind what you know first — the data.** Click the `+` on
   ...
   pipeline feeds `:map` with the output of `:str-split`; lesson 16
   builds one.)
...
- **The `:secret` type-marker** — how taint flows through HOF
  refs without spilling. Lesson 14.
...
[Lesson 08 — Composing pages from components](08-components-and-pages.md)
