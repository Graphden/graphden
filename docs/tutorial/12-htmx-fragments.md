# Lesson 12 — Live fragments: htmx from the graph
...
any route (lesson 08's `:all` list on a self-hosted instance, or
as an app — lesson 28). Open `/clock`, press **Refresh** — the number
...
| Need | Take |
|---|---|
| Click → run a registered JS handler | `:dispatch-action` (lesson 08) |
...
| One-off DOM behaviour no vocabulary covers | `:custom-script` (lesson 09) |
...
[Lesson 13 — Executing a fn: free-arg form, history, cancel](13-executing-a-fn.md)
