# Lesson 40 — AI clients and API tokens: the graph over `/mcp`
...
1. **`create-branch`** `ai/<feature>` — its sandbox, an ordinary
   branch ([Lesson 21](21-branches.md)).
2. **`upsert-fn-defs`** with `fn-defs` as **EDN text** in exactly the
   `fns.edn` shape ([Lesson 29](29-packages.md)) — one string inside
...
3. **`execute-fn`** / **`run-tests`** on that branch — the same
   ...
   ([Lesson 13](13-executing-a-fn.md), [Lesson 15](15-tests.md)).
...
5. **You** open the editor on `ai/<feature>`, press **Δ** against
   ...
   propose, approve, merge ([Lesson 22](22-review.md)).
...
fns, restored on every boot ([Lesson 30 § Fork](30-distributing-packages.md#fork--copy-on-write-when-you-want-to-edit)
...
account's grants ([Lesson 26](26-grants.md)) *intersected* with its
...
1. Create the sandbox branch:
   ...
   curl -s $BASE/mcp -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
     -d '{"jsonrpc":"2.0","id":1,"method":"tools/call",
          "params":{"name":"create-branch","arguments":{"name":"ai/tutorial-40"}}}'
   # → {"ok":true,"branch":{"name":"ai/tutorial-40",…}}
   ```
...
2. Write one fn-def onto it — EDN text inside the JSON string:
   ...
   curl -s $BASE/mcp -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
     -d '{"jsonrpc":"2.0","id":2,"method":"tools/call",
          "params":{"name":"upsert-fn-defs","arguments":{"branch":"ai/tutorial-40",
          "fn-defs":"[{:name :shout :namespace \"ai-demo\" :parent :str-upper :args {:string {:value \"hello from mcp\"}}}]"}}}'
   # → {"ok":true,"branch":"ai/tutorial-40","fn-ids":["…"]}
   ```
...
3. Run it on that branch:
   ...
   ```bash
   curl -s $BASE/mcp -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
     -H "X-Graphden-Branch: ai/tutorial-40" \
     -d '{"jsonrpc":"2.0","id":3,"method":"tools/call",
          "params":{"name":"execute-fn","arguments":{"name":"shout","args":{}}}}'
   # → {"status":"succeeded","result":"HELLO FROM MCP",…}
   ```

4. Read it back as `fns.edn` (`read-fn` with `"branch":"ai/tutorial-40"`)
   — the text you get is the text you sent, plus its parent's row.
5. Open the editor, switch to `ai/tutorial-40` in the branch chip:
...
6. Clean up: delete `ai/tutorial-40` from the branch popover (it was
...
- **Traces over MCP** — `execute-fn` with `trace: true` returns the
  ...
  [Lesson 16](16-debugging-traces.md).
