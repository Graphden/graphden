...
(async () => {
  ...
  // Get all three :get instance nodes bounding boxes
  const ring = await page.evaluate(() => {
    const targets = ['ring-method-kw','ring-query-string','ring-body-input-stream','ring-uri','ring-headers','router-response-body','router-response-headers','router-response-status'];
    return targets.map(t => {
      const os = Array.from(document.querySelectorAll('.node-overlay[data-original-fn-id]')).filter(o => {
        ...
        return l && l.textContent.trim() === t;
      });
      if (!os.length) return { fn: t, found: false };
      const boxes = os.map(o => o.getBoundingClientRect().toJSON());
      return { fn: t, found: true, count: os.length, boxes };
    });
  });
