# Lesson 37 — Plans & tiers: what the cloud grants each account
...
Lesson 16) and its **quota** ceilings (fns, list items, outbound
...
| `dedicated` (paid) | ✓ | ✓ | 5,000 | ∞ | its own pod + always-on services (lesson 35) |
...
**Free (registered)** is what *signing up* gives you (lesson 36),
...
everyone; see lesson 16 + [SECURITY_MODEL.md](../SECURITY_MODEL.md)).
...
`:org` is a tenant-forbidden entity (lesson 27), so tiers are an
...
that app gets its own org (you can belong to several — lesson 33) and
...
1. Sign up a new account (lesson 36) — its org lands on `free`.
...
[Lesson 38 — Services talking to
services](38-services-talking-to-services.md): one service names
