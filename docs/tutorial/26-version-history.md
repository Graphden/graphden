# Lesson 26 — Version history: what changed, and going back
...
Lesson 23 introduced branches; this is the other half of the same
...
- **Branches** (Lesson 23) — the same version rows, read through a
  different branch chain. A merge picks version rows; a `:branch-local?`
  fn's rows deliberately do not travel.
- **Runs** — the Inspector's Runs tab (lesson 15) is the same
