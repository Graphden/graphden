# Lesson 27 — Roles: capabilities as a bundle
...
Lesson 26 drew the line and this lesson picks up the other side of it.
There are **two independent axes** of permission:

| | **Namespace grants** (Lesson 26) | **Org-management capabilities** |
|---|---|---|
...
- **members** — an editable, comma-separated list of **usernames**. A
  ...
  string the Members panel lists them under (lesson 25). It is
...
already reflects it. This is the same argument lesson 03 makes about
`parent-ids` and lesson 30 makes about packages: name the shared thing
...
- **It does not grant namespace access by itself.** A `support` role
  ...
  read `acme.billing`. That is a grant (Lesson 26), and it stays
...
- **It is org-scoped.** A role in `acme` means nothing in `globex`,
  even for the same account (lesson 31).
...
3. Invite a second person if you have not already (lesson 25), then
...
- **Members** (lesson 25) — who is in the org at all; a role only
  matters for someone already in it.
- **Grants** (Lesson 26) — the other axis, per namespace.
- **Across organizations** (lesson 31) — roles do not travel; each org
  answers for itself.
