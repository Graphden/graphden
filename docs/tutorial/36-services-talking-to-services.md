# Lesson 36 — Services talking to services: the contract lives in the graph
...
**You need**: lesson 33 (a service is a no-arg fn the reconciler keeps
alive) and lesson 21 (branches) — a self-hosted instance, or a
...
1. **The producer.** Add these fn-defs the way you add any (the editor,
   ...
   or a package module, lesson 29). Pick a port nothing else uses:
...
2. **Make it a service.** Click `⚙` on `:orders-service` → "Make
   service" → *Create & reconcile* (lesson 33). The reconciler starts
...
4. **Run it.** ▶ on `:fetch-orders` — the Run pane shows
   `{:orders [1 2 3]}`. Trace (lesson 16) shows the chain:
...
5. **Stop the producer.** Disable the service (`⚙` popover, or
   ...
   (the reconciler brings the producer back on its own; lesson 33's
   restart policy).
...
IDL, because a type *is* a fn here (lesson 06). With two teams in one
org, the contract namespace belongs to the producer's team and the
consumer's team holds `read` on it (lesson 26 — a role can be the
...
- **Mutual calls** are legal: `svc-a` may name `svc-b` and `svc-b`
  ...
  so the cycle rule (lesson 11) does not fire.

## Following a call across services

Trace (lesson 16) shows the call tree of one execution, and in step 4
...
- Asynchronous work between services is the next lesson
  ([36 — Queues](37-queues.md)).

## Next

[Lesson 37 — Queues](37-queues.md): asynchronous work between
services.
