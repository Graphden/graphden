...
4b. self-serve — тенант ставит СВОЙ handler.
    ...
    - *Остаётся (механическая обвязка, зеркало app-router):* `:set-org-handler` seam на ctx (create-context/system/addon) + core base-fn `invoke-set-org-handler` + route `POST /api/my-app/handler` + editor-JS «set as app handler».
