...
| [docs/FLEET_RFC.md](docs/FLEET_RFC.md) | **RFC / PROPOSAL (not shipped)** — dynamic fleet: load-based placement + rebalancing. The core tension (static org sharding is brainless; rebalancing needs a live routing map + dynamic membership + a placement controller), the **cell** as the unit of placement (root fn + ref-closure, not the whole org; single-call distributed execution is a non-goal), the two orthogonal axes (pod footprint via GraalVM vs placement model), chosen direction (own control plane + slim-down track), k8s/Knative role *below* the brain, and the 0-to-3 phased plan | Before proposing ANY dynamic-placement / autoscaling / rebalancing work, or picking a k8s/Knative/serverless substrate |
