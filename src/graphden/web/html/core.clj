...
(defbase html-handler
  "Creates a Ring handler that returns HTML response.

   Arguments:
   - body: HTML string or hiccup data structure to return

   Returns:
   Ring handler function (ignores request, returns static HTML).

   Usage in fn-defs:
   {:name :home-handler-fn
    :parent :html-handler
    :args {:body :home-page-hiccup>}}"
  {:args {:body :any}
   :return-type :fn}
  (let [html-str (if (string? body)
                   body
...
;; =============================================================================
;; HTMX Support Functions
;; =============================================================================

(defbase htmx-fragment
  "Renders an HTMX partial (fragment) without full HTML wrapper.

   Arguments:
   - hiccup: Hiccup data structure for the fragment

   Returns:
   Ring response with just the fragment HTML (for hx-swap).

   Use this for HTMX partial updates where you don't want
...
(defbase htmx-handler
  "Creates a Ring handler that returns HTMX fragment.

   Arguments:
   - hiccup: Hiccup data structure for the fragment

   Returns:
   Ring handler that returns the fragment as HTML response."
  {:args {:hiccup :jsonb}
   :return-type :fn}
...
(def all-defs
  ...
  {:render-hiccup render-hiccup
   ...
   :htmx-fragment htmx-fragment
   :htmx-handler htmx-handler
   :html-page html-page
   :with-htmx with-htmx
