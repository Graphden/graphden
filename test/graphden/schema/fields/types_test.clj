...
(deftest types-test
  (testing "all expected types are defined"
    (is (= #{:uuid :text :int :bool :numeric :timestamptz :jsonb :bytes :sequence :keyword}
           ft/supported-types)))
...
(deftest type-mappings-test
  ...
  (testing "mappings are pg-type strings, :custom, or :junction"
    (doseq [[t pg-type] ft/type-mappings]
      (is (or (string? pg-type) (= :custom pg-type) (= :junction pg-type))
          (str "Mapping for " t " should be string, :custom, or :junction"))))

  (testing "can look up specific mappings"
    (is (= "UUID" (get ft/type-mappings :uuid)))
    (is (= "BIGINT" (get ft/type-mappings :int)))
    (is (= "UUID" (get ft/type-mappings :ref)))))
