# Lesson 33 — Services: long-running fns supervised by graphden
...
2. `⋯` → **+ Extend** on the `const` card; name it `tutorial-tick`,
   **Save**. The child takes `const`'s place in `:body` and you stay
   ...
   (lesson 07). Click it, **Bind literal**, and type `tick` — the
...
cross-merge — see lesson 21). Two `:service` rows:
...
1. Pre-req: complete the per-branch web-server walk-through in
   lesson 21. You should have a copy of `:web-server` on
...
- **The free / network tiers are a full FaaS *without* services.** You compose
  fns, deploy a live app at `<label>.graphden.app` (lesson 28), and execute on demand — but
...
diagnostics refuses to execute; see Lesson 13.)
...
[Lesson 34 — Signing up & signing in](34-signing-up-and-in.md): your
account on a graphden cloud. One service naming and calling another
over HTTP is [lesson 36](36-services-talking-to-services.md).
