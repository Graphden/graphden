...
| `editor-packages.js` | Packages sidebar section (PACKAGE_DISTRIBUTION 3e). Server-rendered via `GET /partials/packages-panel`: the current branch's `:package-install` pins (per-row `×` uninstall) PLUS a native `<details>` "browse" of the registry index (each published version an Install button). All mutations (uninstall / install) hx-post / hx-delete with the pkg name+version as query params and swap the refreshed `[data-packages-panel]` root (`hx-swap="outerHTML"`) — install/uninstall transition the installed table ↔ empty-state. JS mounts the partial + owns the collapsible section lifecycle only. NOT tenancy-gated (packages exist single-tenant). Backed by `/api/packages/{installed,uninstall,panel-install}` (the panel's HTML-returning variants; the JSON `/api/packages/{install,update,fork}` remain the programmatic API). |
