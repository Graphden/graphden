...
| `editor-packages.js` | Packages sidebar section (PACKAGE_DISTRIBUTION 3e). Server-rendered via `GET /partials/packages-panel` (read-only table Package \| Version of the current branch's `:package-install` pins); JS mounts the partial + owns the collapsible section lifecycle. NOT tenancy-gated (packages exist single-tenant). Backed by `/api/packages/installed`. |
