...
метадата, в рантайме не используется.
...
- «У пакета может измениться только impl» → это про impl-канал: новая версия
  Maven/git-координаты пакета, сигнализируется семвером/git-тегом.
...
**FaaS/PaaS-дуальность (выпадает из дизайна, без форка).** FaaS — это режим
ОБЛАЧНОГО ТЕНАНТА. Self-hosted (без аддона) и оператор облака (public org)
остаются **PaaS** — полный доступ к `:http-server`/порту/`:service`. Это ровно
ось «аддон вкл/выкл + public/tenant»: `tenant-forbidden`-гвард срабатывает
ТОЛЬКО для `org ≠ public`, так что один кодбейс даёт PaaS self-hosted'у +
оператору и FaaS облачному тенанту — цель §3.0 «работает С аддоном и БЕЗ».
...
**R2 — ЗАКРЫТО.** Расследование показало, что премиса плана была наполовину
устаревшей: `:list-append`/`:list-closed` + персональные неймспейсы + workspaces
**уже полностью построены** (поле + чтение + энфорсмент). Реально не хватало:

- **`:terminal`** — явный seal-флаг на binding (`validation/terminal-rej`,
  обобщает авто-seal `value-override-rej` на ещё-не-valued template-слоты).
- **`:bind-args` / `:append-list`** — две узкие capability + энфорсмент. Шов
  write-authz расширен с `:fn`-only на `:binding`/`:binding-list-item` И на
  **deletes** (`guard-write!` теперь получает entity-id; `delete-entity`
  гардится). Требуемая cap сужается по правке: `:bind-args` для value-only,
  `:append-list` для list-item, `:write` для структурных/create/delete.
  `:write`/`:admin` субсюмят узкие (`grant/cap-implies?`). own = by-namespace.

Доказано: grant-test, authz-test, storage-integration (end-to-end через
реальный `sp/*` — Risk 1). §4.3 готово.
...
| #      | Риск / нестыковка                                                                                                                              |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **R1** | ЗАКРЫТО. Рантайм-гейт эффектов построен (двухслойный ctx-based, см. §5 «Wiring — СДЕЛАН»): `cloud-request-allowed-effects` на запросе + `default-cloud-allowed-effects` на exec-ctx тенанта; тег `:raw-sql` отдельно блокирует сырой pg по платформенному пулу. |
...
| **R3** | ЗАКРЫТО. `export-graph` base-fn + `GET /api/export/graph` (+ `/api/export/graph-rows` для BYO-бутстрапа); publish/export идут через него. |
...
| **R6** | ЗАКРЫТО. Reads org-scoped через `tenancy/storage` (`default-scoped-entities`) + RLS как нижний предохранитель (`tenancy/rls.clj`, policy SELECT own+public / write own-only). |
...
| **R9** | ЗАКРЫТО (org-скоуп). Секрет = `:fn` с родителем `:secret-leaf`, а `:fn` уже org-scoped (`default-scoped-entities`) → чужие секреты невидимы; rotate/delete доп. защищены org-guard'ами (`crud/secrets.clj`). Остаётся при желании namespac'ить сами vault-пути и добавить user-level (не только org-level) скоуп. |
...
| **R13**| ЗАКРЫТО (per-user построен). Таблица `:user` + токены (SHA-256 + bcrypt-12 + TTL) + grants + проброс org-ctx (`tenancy/users.clj`). Общий `AUTH_TOKEN` остаётся только как single-tenant fallback вне tenancy-аддона. |
...
- **Фаза 2–3 (tenancy-аддон, по §3.0):** строго в порядке предусловий —
  ...
  3. **`OrgScopedStorage` + RLS** (R6, оба слоя) — **ПОСТРОЕНО И
     ...
     *Остаётся чистый деплой/инфра:* приложение под non-superuser ролью
     ...
     static-map). Дальше по плану: per-org LRU (R8) → поддомены (R10, R11).
     (Org-скоуп секретов R9 — уже сделано, см. §7.)
  ...
  5. **`grant`-примитив** (§4.2) — **ПРИМИТИВ ГОТОВ**:
     ...
     - **workspaces (backend)** `grant/workspace` — union именованных
       ...
       sibling нет).
     *R2 (`:terminal` seal / `:list-append`) — сделано, см. §7.*
