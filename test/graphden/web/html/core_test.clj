(ns graphden.web.html.core-test
  (:require
    [clojure.string :as str]
    [clojure.test :refer [deftest is testing]]
    [graphden.web.html.core :as html]))
...
(deftest html-handler-test
  (testing "creates handler that returns HTML"
    (let [handler (call-impl html/html-handler {:body [:p "Hello"]})]
      (is (fn? handler))
  ...
  (testing "handler ignores request"
    (let [handler (call-impl html/html-handler {:body "Static"})]
...
(deftest htmx-fragment-test
  (testing "renders fragment without DOCTYPE"
    (let [response (call-impl html/htmx-fragment {:hiccup [:div {:id "result"} "Updated"]})]
      (is (= 200 (:status response)))
      (is (not (str/includes? (:body response) "<!DOCTYPE")))
      (is (str/includes? (:body response) "<div"))
      (is (str/includes? (:body response) "Updated")))))


(deftest htmx-handler-test
  (testing "creates handler for HTMX fragments"
    (let [handler (call-impl html/htmx-handler {:hiccup [:span "Fragment"]})]
      (is (fn? handler))
      (let [response (handler {})]
        (is (= 200 (:status response)))
        (is (str/includes? (:body response) "<span>Fragment</span>"))))))
...
(deftest all-defs-test
  (testing "contains all expected functions"
    ...
    (is (contains? html/all-defs :html-handler))
    (is (contains? html/all-defs :htmx-fragment))
    (is (contains? html/all-defs :htmx-handler))
