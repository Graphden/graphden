...
(defprotocol CacheStrategy
  ...
  ;; Cache access
  (get-cached [this cache-key]
    "Get cached value for key, returns nil if not cached")

  (compute-if-absent [this cache-key compute-fn]
...
(defn root-ancestor-key [node-name]
  [:root-ancestor node-name])

(defn full-args-key [node-name]
  [:full-args node-name])

(defn children-key [node-name]
  [:children node-name])

(defn arg-refs-key [node-name]
  [:arg-refs node-name])
