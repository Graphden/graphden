# Lesson 29 — Roles: capabilities as a bundle
...
Lesson 28 drew the line and this lesson picks up the other side of it.
There are **two independent axes** of permission:

| | **Namespace grants** (Lesson 28) | **Org-management capabilities** |
|---|---|---|
...
- **members** — an editable, comma-separated list of **usernames**. A
  ...
  string the Members panel lists them under (lesson 27). It is
...
`parent-ids` and lesson 32 makes about packages: name the shared thing
...
- **It does not grant namespace access by itself.** A `support` role
  ...
  read `acme.billing`. That is a grant (Lesson 28), and it stays
...
- **It is org-scoped.** A role in `acme` means nothing in `globex`,
  even for the same account (lesson 33).
...
- **Members** (lesson 27) — who is in the org at all; a role only
  matters for someone already in it.
- **Grants** (Lesson 28) — the other axis, per namespace.
- **Across organizations** (lesson 33) — roles do not travel; each org
  answers for itself.
