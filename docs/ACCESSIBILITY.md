...
| Explorer filters | The kind chips are a `role=group` of `aria-pressed` toggles; every dynamic chip (`#gd-filter-chips`) is a pressed toggle whose accessible name says *Remove filter: …* (and *no longer exists* when it names a deleted fn / view); `◍ all` clears everything. The **+ filter** menu and the view chip's popover are non-modal dialogs (`installPopoverDismiss` + `ensurePopoverClose`, focus in on open, Escape / × out, focus back to the chip); the namespaces and graph-view rows are `role=checkbox` with `aria-checked`, the *Name contains* field adds its chip on Enter. Every change calls `gdAnnounce` — *Only types — 12 of 40 shown*, *Only core*, *core.tests hidden*, *View on-const*, *N functions match*, *All functions*. Leader: `Space f f` add a filter, `Space f v` views, `Space f a` all. | `editor-explorer-filters.js`, `editor-explorer-views-ui.js`, `editor-sidebar-lens.js` |
...
| Management surfaces (Settings / Organization / Platform / Marketplace) | Dialog-like entry: `gdAnnounce` names the surface, focus moves to its section nav, the covered Build chrome inside `#main-container` goes `inert` (the top bar stays live — brand/org/chip work from any surface), Escape returns to Build (`window`-level, `defaultPrevented`-guarded so dialogs close first; from a text field on the surface the first Escape only leaves the field for the `#gd-surface-exit` button, the second — or Enter — leaves the surface), and the `#gd-surface-exit` button is the pointer counterpart — it lives in the bar, the one region that stays live, and is placed over the sheet's top-right corner by CSS | `editor-shell.js` |
| Undo toast | `#gd-undo-toast` (`editor-undo.js`): `role="status"` + `aria-live="polite"`, so "Created foo · Undo · ×" (shown for 10 s, held while the pointer or focus is on it) is announced without moving focus — the reader is mid-gesture and must not be pulled to a toast. Its two buttons are ordinary focusable buttons (Tab reaches them; the Undo button's `aria-label` names the gesture), and the same undo stays on the keyboard for the whole 30 s — `Space u`, or the `Mod+z` chord (inert inside a text field, where the browser's own undo keeps it) — so no one has to catch a toast to undo | `editor-undo.js` |
