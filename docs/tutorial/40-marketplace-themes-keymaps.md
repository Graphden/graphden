# Lesson 40 — The Marketplace: themes, keyboard layouts, and what others published
...
[Lesson 32](32-distributing-packages.md) left you with a registry: a
...
|---|---|---|
| **Packages** | fns-only packages published from a namespace | Install (a per-branch pin) or Fork (editable copies — [Lesson 32 § Fork](32-distributing-packages.md#fork--copy-on-write-when-you-want-to-edit) says when it refuses) — exactly Lesson 32's two doors |
...
> packages ship base-fns here?* An impl+fns package (Lesson 31) is
...
The ⬆ **publish** popover on a namespace row (Lesson 32) now has three
...
What if the package came from *another* graphden? Lesson 32's
...
navigation contract ([lesson 21](21-keyboard-and-accessibility.md)), not
...
6. **Reset to defaults** on the Keyboard pane, **Reset to built-in** on
   ...
   Lesson 32 if you want them gone).
...
[Lesson 41 — The package lifecycle](41-package-lifecycle.md): the loop
