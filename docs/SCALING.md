...
| A skipped/lost invalidation self-heals (client abort mid-request, a write path with no NOTIFY, a dropped NOTIFY) | graph EPOCH: a PG sequence bumped BEFORE every graph-shaped write; the branch-router validates its cached ctxs against it on fetch (TTL ≈1s). A heal rebuilds the BASE ctx in place (stale-while-revalidate) and DROPS every other cached branch ctx — rebuilt on its next request — so a heal costs one compile, not one per cached branch (merged source branches are permanent, so a workspace holds many); a branch with a RUNNING service is pinned (the reconciler registers `set-pinned-branches-fn!`) and rebuilds in place too, so the service keeps the ctx the router serves. Every heal logs its reason (`graph-epoch heal reason` — the aborted epochs with the entity that bumped them, or the foreign ones). An un-noted local bump counts as aborted only after 45 s, above the 30 s abort-shield budget, so a merely slow write never triggers a heal; the boot package sync notes its own bumps (`note-all-applied!`) so a fresh boot does not heal once for nothing, a never-called sequence reads as epoch 0, and `dispatch` binds the request bump log itself when its caller did not. Ledger entries survive the watermark for five minutes (`*ledger-retention-ms*`) so a second epoch STATE that is behind (the parallel test plugin isolates one per NS thread) still classifies them. Eager invalidate + NOTIFY stay as the fast path; correctness no longer depends on them. Under the tenancy addon too: every ledger call resolves the pool-bearing handle beneath the org-scoped decorator (`epoch-handle`), and the heal's deleted-branch check reads the raw handle at platform context | `storage/postgres/graph_epoch.clj`, `system/branch_router/epoch.clj` `validate-graph-epoch!` |
...
- **SSE invalidation — DONE.** The BYO executor has no PG connection, so it
  ...
  - Fan-out is per-org: `crud.entities/notify-after-write!` tags each
    ...
    registers each subscriber under its org — the principal's own `:org`, or
    (the org-agnostic accounts principal) the org the exec ctx's
    request-scope resolves from the account's memberships, narrowed by the
    `gd_org` selector the BYO executor sends for `GRAPHDEN_EXECUTOR_ORG` —
    and an event goes
