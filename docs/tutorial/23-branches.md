# Lesson 23 — Branches: fork, edit, diff, merge
...
[lesson 24](24-review.md)'s subject; its knobs live in the same
**⚙ Protection…** dialog.
...
10. From `main`, click `⇢` next to `tutorial-branch`. Confirm. The
    ...
    history and can no longer be deleted — [lesson 24](24-review.md)
    explains why.)
...
type diagnostics (what the Explorer's ⚠ type errors filter marks — Lesson 19),
...
- Per-branch services (running `:my-web-server` as a managed
  service on dev while `:web-server` runs on main) — Lesson 35.
...
[Lesson 24 — Review: propose, approve, and the protected merge](24-review.md):
