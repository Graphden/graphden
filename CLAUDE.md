...
| [docs/FLEET_RFC.md](docs/FLEET_RFC.md) | **RFC / PROPOSAL (not shipped)** — dynamic fleet: load-based placement + rebalancing. The **cell** (root fn + forward ref-closure) as placement unit (services are already cells; an org app is one cell today; single-call distributed execution is a non-goal); dynamic membership reuses the live compiled-registry atom + `delta-recompile!`/`compile-subset` (only a forward-closure walk + evict-refcount are new); footprint/start track = **CRaC-first, GraalVM shelved** (eval-at-boot package model blocks native-image); §9 spells out that graph **hot-reload is already shipped** and this adds placement not freshness; own control plane, k8s with Knative/HPA below the brain; phased 0-3 with concrete T2.1-T2.6 | Before proposing ANY dynamic-placement / autoscaling / rebalancing / hot-reload / native-image work, or picking a k8s/Knative/serverless/CRaC substrate |
