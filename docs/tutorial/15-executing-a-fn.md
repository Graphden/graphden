# Lesson 15 — Executing a fn: free-arg form, history, cancel
...
Lesson 35 introduces SERVICES — fns graphden keeps running
...
per category, `:db`, `:network`, `:env` and the rest; lesson 16
...
optionally what each returned. That is lesson 18's subject — the
canvas highlight, the step-through call tree, and the capture caps.

## What we glossed over

- **Branch-aware execution** — the active branch picks which
  version of the fn-graph runs. Lesson 23 covers branches.
- **Service-mode execution** — fns marked as services run
  forever, supervised by graphden. Lesson 35.
- **HOF call shape** — how internal refs get their free args
  bound at compile time vs call time. Lesson 09.

## Next

[Lesson 16 — Effects and the `:secret` type-marker](16-effects-and-secrets.md)
