# Lesson 11 — Recursion: loops without cycles
...
> [open the demo with the tour running](https://app.graphden.dev/?demo=1&tutorial=11)
...
Lesson 07 covered the collection case: `map`, `filter` and friends
...
1. **`{:as :input}` and `{:as :self}`** are how the step reads its
   call-site args. They are free args (lesson 05) filled by `:fix` at
...
- **A sequence you already have** → `:map` / `:filter` / `:reduce`
  (lesson 07). They are clearer and the layout shows the shape.
- **A long-lived loop with effects** → a service (lesson 33).
...
- **Services** (lesson 33) — the other kind of repetition: effectful,
  supervised, and stopped by an interrupt rather than a base case.
- **Tests** (lesson 15) — a recursive fn is worth an `:assert-eq`
  covering the base case AND one recursive step; those are the two
  places this shape goes wrong.

## Next

[Lesson 12 — Live fragments: htmx from the graph](12-htmx-fragments.md)
