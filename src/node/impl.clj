...
(defrecord Node
  [node-name parent-name args fast-refs])
  ...
  (if (map? args)
    ...
    (let [init-arg #(if-let [parent-node-name (:parent-node-name %)]
                      (if (= parent-node-name node-name)
                        (arg-impl/init %)
                        (thrw "Incorrect arg parent-node-name"
                              {:node-name node-name
                               :arg %
...
(defn init
  ([{:keys [node-name parent-name args fast-refs]}]
   (->Node node-name
           parent-name
           (prepare-args node-name
                         parent-name
                         args)
           (fast-refs-impl/init fast-refs)))
  ...
  ([node-name parent-name args fr]
   (let [fr (if fr
              (fast-refs.impl/init fr)
              (fast-refs.impl/init))]
     (->Node node-name
             parent-name
             (prepare-args node-name
             ...
             (if parent-name
               fr
               (fast-refs/set-full-args fr args nil))))))
...
(defn delete-arg-back-ref-node
  ...
  (init (update this
                :fast-refs
                fast-refs/delete-arg-back-ref-node
                node-name
                arg-name)))
...
(defn set-base-node-name
  [{:keys [parent-name] :as this} base-node-name]
  (if parent-name
    (init (update this
                  :fast-refs
...
(defn set-full-args
  [{:keys [args] :as this} parent-full-args]
  (init (update this
                :fast-refs
...
(defn rename-child-back-ref
  [this child-name new-child-name]
  (-> this
