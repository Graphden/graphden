...
| Undo toast | `#gd-undo-toast` (`editor-undo.js`): `role="status"` + `aria-live="polite"`, so the 30-second "Created foo · Undo · ×" is announced without moving focus — the reader is mid-gesture and must not be pulled to a toast. Its two buttons are ordinary focusable buttons (Tab reaches them; the Undo button's `aria-label` names the gesture), and the same undo is on the leader as `Space u`, so no one has to catch a toast to undo | `editor-undo.js` |
