...
| [docs/USER_SITES_PLAN.md](docs/USER_SITES_PLAN.md) | Roadmap for enabling users to build their own sites end-to-end in the graph — Blocks 1 (runtime + DSL), 2 (starter component library + `/demo/contact`), and 3 (`:custom-script` escape hatch — `:js-source` type, `:wrap-custom-script`, `:dispatch-custom`, `_form-js-source` widget) all shipped 2026-06-23. Remaining: Block 4 user-site routing + tutorial. | When working on `editor-runtime.js` / `editor-actions-builtin.js`, the `web/runtime` or `web/components` packages, adding a new component or escape-hatch primitive, or extending the user-site demo |
