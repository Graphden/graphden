...
| `:unreferenced-private` | a `_`-private composed fn-def or type-row no fn-def references (parents, args, list items, return types, type-row fields; string names count — the value-form / repr registries hand names out as strings). A private shape a base-fn declaration names as an arg or return type is referenced too: the corpus gate roots the names base-fn declarations use (`lint/names-referenced-by`). | warning |
| `:unreferenced-public` | **corpus gate only** (`bb graph-lint`, `:check-public? true`) — a PUBLIC composed fn-def nothing outside a `tests` namespace references is shipped vocabulary with no witness. The finding asks for a caller, or a `:vocabulary` line in `tools/graph-reachability.edn` naming the lesson or doc the name is for (the surface audit found nine such names that had quietly died; a public fn-def used to be a live root by definition). The editor's ⚐ lens never files it: in a user's graph a public fn-def IS the entry point. | warning |
