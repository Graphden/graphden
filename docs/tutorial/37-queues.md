# Lesson 37 — Queues: asynchronous work between services
...
**You need**: lesson 33 (services) and lesson 36 (two services
talking over HTTP) — this lesson is the asynchronous counterpart.

## Why a queue, and why Postgres

In lesson 36 the consumer *called* the producer and waited. That is
...
3. **Run the consumer.** `⚙` on `:orders-worker` → "Make service" →
   *Create & reconcile* (lesson 33). Within a second the pending
...
6. **Follow a message across the queue.** Run `:order-placed` from the
   Run pane (a persisted run has an identity — lesson 36) and open the
...
As with HTTP (lesson 36), put the message's shape in a type-row both
sides reference:
...
[Lesson 38 — The Marketplace](38-marketplace-themes-keymaps.md):
