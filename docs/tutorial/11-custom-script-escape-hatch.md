# Lesson 11 — The `:custom-script` escape hatch
...
Lesson 10 covered the platform components and the two
...
Lesson 10 — `:html-page` alone gives you hiccup, not a route):
...
Mount it (Lesson 10), open `/tips`, hover over any `[data-tip]`
...
`:custom-button` (in `web.components`, Lesson 10) is the
...
refinement (Lesson 08) of `[:list :any]` whose first element must
...
[Lesson 12 — State: cells, swap, and a graph-native cache](12-state-cells-and-caches.md)
