...
| `editor-runtime.js` | Platform-shared client primitives — `registerActionHandler(action, fn)` / `bindActionDispatch(host)` / `loadPartial(host, url, opts)`. Extracted from `editor-row-actions.js` during the HTMX Phase-A migration so component fn-defs / partials / graph-composed pages reuse the same dispatch / fetch-and-swap surface. Sandbox-tested via `tools/runtime-test/runtime.test.js` (no browser). |
| `editor-actions-builtin.js` | Three platform-provided action handlers: `navigate` (sets `window.location.href` from `data-href`), `submit-form` (finds the nearest `<form>` ancestor, POSTs via fetch, swaps the response into `data-target` or back into the form), and `custom` (evaluates `data-custom-handler` as `(btn, event, host) => …`). Registered via `editor-runtime.js`'s `registerActionHandler`. Sandbox-tested in `tools/runtime-test/actions-builtin.test.js`. |
