# Lesson 39 — The package lifecycle: both sides of a version
...
This lesson assumes [Lesson 30](30-distributing-packages.md) (publish /
install / update / fork / withdraw, one action at a time) and
[Lesson 22](22-review.md) (propose → approve → merge). Here they are
...
2. Open the **packages** chip, expand **+ Install a package**, click
   **Install** on `acme-greet 1.0.0`. The pin appears in the table; the
   ...
   ([Lesson 30 § Install](30-distributing-packages.md#install--by-reference-not-by-copy)).
...
author's trunk and propose it, exactly Lesson 22:
...
another graphden ([Lesson 30 § another registry](30-distributing-packages.md#installing-from-another-graphdens-registry)),
...
- **When Fork refuses** — a namespace built into *this* instance from
  ...
  [Lesson 30 § Fork](30-distributing-packages.md#fork--copy-on-write-when-you-want-to-edit).
...
| running a hub you can reach | push your branch: `push/<branch>`, owner-stamped, reviewed on the hub with Δ compare ([Lesson 32 § 3](32-offline-and-push.md#3-push-your-work-to-the-hub)) |
...
1. On `vendor`, hover `acme.greet`, **⬆**, `acme-greet` `1.0.1`,
    ...
    refused under `1.x` and need `2.0.0` — [Lesson 30 § Publish](30-distributing-packages.md#publish--freeze-a-namespace-into-the-registry).)
...
2. Before withdrawing anything on a real registry, look at the
    **Organization** surface's packages section
    ([Lesson 30 § Governance](30-distributing-packages.md#governance--the-organization-surfaces-packages-view)):
...
into the popover's Merged group ([Lesson 21](21-branches.md)).
...
| A package built into the instance | Neither door: the change is a `fns.edn` edit and a `bb rebuild` (Lesson 29). |

## What we glossed over

- **Marketplace metadata** — description, category, tags, the ★
  rating and the ↓ install count the author sees: [Lesson 38](38-marketplace-themes-keymaps.md).
- **Suggested changes** on a proposal (a reviewer's counter-edit,
  applied with one click) and anchored review comments: [Lesson 22](22-review.md).
...
- **Secrets in a package** — a fn that reads a secret publishes with a
  ...
  ([Lesson 14](14-effects-and-secrets.md)).

## Next

[Lesson 40 — AI clients and API tokens](40-ai-clients-and-api-tokens.md):
