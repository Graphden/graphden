...
- Memory: one ExecutionContext per active branch, bounded by an **LRU
  ...
  via `GRAPHDEN_MAX_CACHED_BRANCHES`); a cached entry idle for 15 min
  (`*ctx-idle-ttl-ms*`) is dropped on the next request's sweep and
  rebuilt when its branch is next used. Adding a branch beyond the cap
...
editor therefore phrases those as "only on <branch>" (the − badge /
...
|------------|-------|--------------|
| Branch chip | context bar (`#gd-ctxbar` → `#branch-mount`, between the view chip and the packages chip) | Shows current branch. Inverted style when off main. |
