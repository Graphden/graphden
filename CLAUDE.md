...
| [docs/USER_SITES_PLAN.md](docs/USER_SITES_PLAN.md) | Roadmap for enabling users to build their own sites end-to-end in the graph — all four v0 blocks shipped 2026-06-23: Block 1 (runtime + DSL), 2 (starter component library + `/demo/contact`), 3 (`:custom-script` escape hatch), 4 (`app.user-site` templates + `:user-site-routes` mount-point + Lesson 14). Remaining v1 follow-ups (multi-site, branch-scoped routing, auto-discovery of user route packages, form-data echo in demo) tracked at the bottom of the doc. | When working on `editor-runtime.js` / `editor-actions-builtin.js`, the `web/runtime` / `web/components` / `app/user-site` packages, adding a new component or escape-hatch primitive, or extending the user-site demo |
