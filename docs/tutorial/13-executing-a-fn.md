# Lesson 13 — Executing a fn: free-arg form, history, cancel
...
Lesson 33 introduces SERVICES — fns graphden keeps running
...
see lesson 05), the Run pane shows a form to supply
...
- Record / list slots → nested form (see lesson 06)
...
per category, `:db`, `:network`, `:env` and the rest; lesson 14
...
errors filter — Lesson 04) is refused at submit. The run comes back rejected
...
optionally what each returned. That is lesson 16's subject — the
canvas highlight, the step-through call tree, and the capture caps.

## What we glossed over

- **Branch-aware execution** — the active branch picks which
  version of the fn-graph runs. Lesson 21 covers branches.
- **Service-mode execution** — fns marked as services run
  forever, supervised by graphden. Lesson 33.
- **HOF call shape** — how internal refs get their free args
  bound at compile time vs call time. Lesson 07.

## Next

[Lesson 14 — Effects and the `:secret` type-marker](14-effects-and-secrets.md)
