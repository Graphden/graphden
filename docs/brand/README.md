...
- `gd-banner-boosty.png` — Boosty cover, 1500x500 (Ink ground, λ mark + wordmark + tagline).
- `gd-banner-x.png` — X/Twitter header, 1500x500 (Soft ground, text-free ambient graph — the avatar overlap clips nothing).
...
`org-banner` colorways) are generated by `gen_banners.py` (PIL; stock
DejaVu fonts). The tagline they carry is the canonical one-liner from
[PHILOSOPHY § Positioning](../PHILOSOPHY.md#positioning) — when it changes,
edit `TAGLINE` in the script, re-run
