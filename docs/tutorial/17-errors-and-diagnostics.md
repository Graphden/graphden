# Lesson 17 — When something breaks, and when it just repeats: the problem filters
...
the graph, and the diagnostic rides alongside it (lesson 06 — "a
...
All three are **filter chips** in the Explorer (Lesson 18): click a chip
...
▶ with "Save to history" unticked (lesson 13) — leaves no audit row, so
...
displayed — so a secret that flowed into an exception (lesson 14) is
...
That is the same text lesson 06 showed you on a single card's badge.
...
Why the writes were allowed at all is lesson 06's point, worth
...
- **fan-in-extract-parent** — two or more fns inherit the same parent
  and bind the same values, differing elsewhere. Lesson 03's move:
...
child (lesson 05); a duplicate is a finding only when the shared
...
The fix is lesson 03's move: keep one definition and let the other
...
Tests are the fourth problem-style filter — **✓ tests** (lesson 15) — and the trace
tree with the «catch next request» trap (lesson 16) live on the
...
strip in the Runs tab reads per fn, and the one lesson 35's plan
ceilings are measured against ([MONITORING.md § 1](../MONITORING.md)).

## Where this shows up next

- **Tests** (lesson 15) — a failing test is a failed run like any
  other, so the ✕ failed filter marks it too, with its assertion message in
  the Runs tab.
- **Debugging** (lesson 16) — from a failed row, the next question is
  usually "which node threw?", which is what the trace tree answers.
- **Plans** (lesson 35) — the same audit rows feed the usage counters;
  a plan's retention is what decides how far back the ✕ failed filter can look.
- **Packages** (lesson 29) — the same lint runs over graphden's own
