# Lesson 19 — When something breaks, and when it just repeats: the problem filters
...
the graph, and the diagnostic rides alongside it (lesson 08 — "a
...
All three are **filter chips** in the Explorer (Lesson 20): click a chip
...
▶ with "Save to history" unticked (lesson 15) — leaves no audit row, so
...
displayed — so a secret that flowed into an exception (lesson 16) is
...
That is the same text lesson 08 showed you on a single card's badge.
...
Why the writes were allowed at all is lesson 08's point, worth
...
Tests are the fourth problem-style filter — **✓ tests** (lesson 17) — and the trace
tree with the «catch next request» trap (lesson 18) live on the
...
strip in the Runs tab reads per fn, and the one lesson 37's plan
ceilings are measured against ([MONITORING.md § 1](../MONITORING.md)).

## Where this shows up next

- **Tests** (lesson 17) — a failing test is a failed run like any
  other, so the ✕ failed filter marks it too, with its assertion message in
  the Runs tab.
- **Debugging** (lesson 18) — from a failed row, the next question is
  usually "which node threw?", which is what the trace tree answers.
- **Plans** (lesson 37) — the same audit rows feed the usage counters;
  a plan's retention is what decides how far back the ✕ failed filter can look.
- **Packages** (lesson 31) — the same lint runs over graphden's own
