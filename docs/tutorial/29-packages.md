# Lesson 29 — Packages: namespaces, fns.edn, impls.clj, deps
...
| `:services` | Optional — a package can declare a fn to keep running (the `app` package declares `:web-server` this way). See [Lesson 33](33-services.md) |
...
- a **fn-def** — `:parent` / `:parents` + `:args` binding the
  ...
  [01](01-fn-defs.md)–[04](05-free-arguments.md).)
...
strips show the full set. See [Lesson 14](14-effects-and-secrets.md).
...
publish / install / update / fork — is [Lesson 30](30-distributing-packages.md).)
...
3. Add `"mycorp"` to `:package-names` in
   ...
   (Lesson 05); the `{:value …}` items are literal strings joined
...
Unlike a registry install ([Lesson 30](30-distributing-packages.md)),
...
- **Distributing a package as data** (not on disk) — publish it
  into the graph and install it on a branch: [Lesson 30](30-distributing-packages.md).

## Next

[Lesson 30 — Distributing packages](30-distributing-packages.md):
