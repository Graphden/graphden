# Lesson 09 — The `:custom-script` escape hatch
...
Lesson 08 covered the platform components and the two
...
Lesson 08 — `:html-page` alone gives you hiccup, not a route):
...
Mount it (Lesson 08), open `/tips`, hover over any `[data-tip]`
...
`:custom-button` (in `web.components`, Lesson 08) is the
...
refinement (Lesson 06) of `[:list :any]` whose first element must
...
[Lesson 10 — State: cells, swap, and a graph-native cache](10-state-cells-and-caches.md)
