...
| [docs/GRAPH_LINT.md](docs/GRAPH_LINT.md) | The graph linters — duplicate-definition (shallow + after helper expansion), unreferenced-private, private-alias; `bb graph-lint` gate, weighting, the editor's Lint tab + graph-stored suppressions | Before adding a fn-def that looks like an existing one, when `bb graph-lint` fails, or before touching `src/graphden/lint/` or the `_plint-*` partial |
...
| [docs/adr/ADR-inherited-rename-surface.md](docs/adr/ADR-inherited-rename-surface.md) | The inherited-rename SURFACE contract — public names are the closest-chain rename, applied at the boundary; the walker/HOF internals stay per-fid | Before touching free-arg NAMING (`surface-entries`, `rename-for-slot`, `free-args-via`) or proposing rename semantics changes |
