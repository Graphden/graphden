# Lesson 35 — Plans & tiers: what the cloud grants each account
...
Lesson 14) and its **quota** ceilings (fns, list items, outbound
...
| `dedicated` (paid) | ✓ | ✓ | 5,000 | ∞ | its own pod + always-on services (lesson 33) |
...
**Free (registered)** is what *signing up* gives you (lesson 34),
...
everyone; see lesson 14 + [SECURITY_MODEL.md](../SECURITY_MODEL.md)).
...
`:org` is a tenant-forbidden entity (lesson 25), so tiers are an
...
that app gets its own org (you can belong to several — lesson 31) and
...
1. Sign up a new account (lesson 34) — its org lands on `free`.
...
[Lesson 36 — Services talking to
services](36-services-talking-to-services.md): one service names
