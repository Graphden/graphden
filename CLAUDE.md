...
| `editor-layout.js` | Grid layout algorithm, positioning. Also owns `taxiBendX` — the single source of truth for where a taxi edge turns (graph coords). The cytoscape `taxi-turn` style, the edge-hover hit-test, and the edge-label anchor all call it; when they each had their own copy, a dragged node put the label on the wrong side of the bend. |
...
| `editor-overlay-manager.js` | Base `createOverlay` factory, placeholder-overlay binder, `createNodeOverlays` lifecycle. Owns `#graph-layer` — the single div that carries `translate(pan) scale(zoom)` for every overlay at once. Overlays are laid out in **graph coordinates**, so `applyViewportTransform()` (O(1), on `cy.on('pan zoom')`) is separate from `syncOverlayGeometry()` (O(n), only when a node moves or an overlay resizes). `updateOverlayPositions()` calls both; use it after a layout / animation frame / drag, never on pan-zoom. |
