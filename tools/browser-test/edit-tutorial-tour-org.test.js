// Lessons 27, 28, 29, 30, 33, 36, 37 — the ORGANIZATION tours.
//
...
async function lesson16(page) {
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 27 invite-explained Next');

  // The link-invite form step is read-only ("try it or skip it").
  await waitTourTitle(page, 'The other way in: a link', 30000);
  assert(await clickTourButton(page, 'Next'), 'lesson 27 link-invite Next');
  ...
  console.log('  lesson 27: walked — invited, read the row, revoked');
}
...
async function lesson17(page) {
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 28 granted Next');

  await waitTourTitle(page, 'Read the row you just wrote', 30000);
  assert(await clickTourButton(page, 'Next'), 'lesson 28 read-row Next');
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 28 revoked Next');

  await waitTourTitle(page, "That's authorization", 30000);
  await finishTour(page, 'lesson 28');
  console.log('  lesson 28: walked — granted a namespace, read it, revoked it');
}
...
async function lesson21(page) {
  ...
  await nextTimes(page, 5, 'lesson 33');
  await waitTourTitle(page, "That's the map", 30000);
  await finishTour(page, 'lesson 33');
  console.log('  lesson 33: walked (all-manual by design — a one-org account has no switcher)');
}
...
async function lesson19(page) {
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 36 opening Next');
  ...
  await nextTimes(page, 4, 'lesson 36');
  await waitTourTitle(page, 'Leaving', 30000);
  await finishTour(page, 'lesson 36');
  console.log('  lesson 36: walked — Settings, identities, 2FA, tokens');
}
...
async function lesson18(page) {
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 37 opening Next');
  ...
  await nextTimes(page, 2, 'lesson 37');
  await waitTourTitle(page, 'Reading your own tier', 30000);
  await finishTour(page, 'lesson 37');
  console.log('  lesson 37: walked — usage panel, ceiling, tier');
}
...
async function lesson31(page) {
  ...
  assert(await clickTourButton(page, 'Next'), 'lesson 29 created Next');
  ...
  assert(await clickTourAdvance(page, 'Next'), 'lesson 29 member Next');

  await waitTourTitle(page, 'What they can do now', 30000);
  assert(await clickTourButton(page, 'Next'), 'lesson 29 effect Next');
  await waitTourTitle(page, 'What a role is NOT', 30000);
  assert(await clickTourButton(page, 'Next'), 'lesson 29 not-a-grant Next');
  ...
  console.log('  lesson 29: walked — created a role, set its members, deleted it');
}
...
(async () => {
  ...
  console.log('edit-tutorial-tour-org — lessons 27 / 28 / 29 / 30 / 33 / 36 / 37 @ ' + BASE);
