...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    const rootData = nodeMap.get(root);
    
    return {
      rootLabel: rootData?.label,
      rootChildren: rootChildren.map(id => {
        const data = nodeMap.get(id);
        return data ? data.label?.split('\n')[0]?.substring(0, 25) : id;
      })
    };
  });
  
  console.log('Root:', result.rootLabel);
  console.log('\nRoot children (in adjacency order):');
  result.rootChildren.forEach((label, i) => {
    const marker = label?.includes('entity-form') ? ' <--' : '';
