...
function createArgOverlay(node, container) {
  ...
  // Compare-mode ring (UX-v3): when the mode is on and THIS overlay's
  // arg is one of the fn's changed slots, ring it and add a Δ badge
  // whose tooltip carries the here · there summary.
  (function annotateDiffFocus() {
    if (typeof gdDiffSlotsForFn !== 'function') return;
    const argLocal = (typeof argRowFromNode === 'function')
                     ? argRowFromNode(node.data())
                     : null;
