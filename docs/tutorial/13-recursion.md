# Lesson 13 — Recursion: loops without cycles
...
Lesson 09 covered the collection case: `map`, `filter` and friends
...
- **A sequence you already have** → `:map` / `:filter` / `:reduce`
  (lesson 09). They are clearer and the layout shows the shape.
- **A long-lived loop with effects** → a service (lesson 35).
...
- **Services** (lesson 35) — the other kind of repetition: effectful,
  supervised, and stopped by an interrupt rather than a base case.
- **Tests** (lesson 17) — a recursive fn is worth an `:assert-eq`
  covering the base case AND one recursive step; those are the two
  places this shape goes wrong.

## Next

[Lesson 14 — Live fragments: htmx from the graph](14-htmx-fragments.md)
