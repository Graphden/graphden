# Lesson 31 — Roles: capabilities as a bundle
...
Lesson 17 drew the line and this lesson picks up the other side of it.
There are **two independent axes** of permission:

| | **Namespace grants** (lesson 17) | **Org-management capabilities** |
|---|---|---|
...
`parent-ids` and lesson 14 makes about packages: name the shared thing
...
- **It does not grant namespace access.** A `support` role with
  ...
  `acme.billing`. That is a grant (lesson 17), and it stays separate on
...
- **It is org-scoped.** A role in `acme` means nothing in `globex`,
  even for the same account (lesson 21).
...
3. Invite a second person if you have not already (lesson 16), then
...
- **Members** (lesson 16) — who is in the org at all; a role only
  matters for someone already in it.
- **Grants** (lesson 17) — the other axis, per namespace.
- **Across organizations** (lesson 21) — roles do not travel; each org
  answers for itself.
